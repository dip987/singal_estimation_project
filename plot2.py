...
for abs_w in w_space:
    ...
    w = abs_w_rad * np.array([0, 0, 1]).reshape(3, 1)
...
plt.plot(smooth_w_space,
         [crlb_plot_wrapper(w, 'z', 0.01, sig_s, sig_w, Params.num_s, Params.num_w, 2, False) for w in
          smooth_w_space], linewidth=4)
plt.plot(smooth_w_space,
         [crlb_plot_wrapper(w, 'z', 0.01, sig_s, sig_w, Params.num_s, Params.num_w, 2, True) for w in
...
plt.legend(['sqrt(CRLB) along z', 'sqrt(saturated CRLB) along z', '$w_x MLE$', '$w_y MLE$', '$w_z MLE$', 'average w_x', 'average w_y',
            'average w_z'])
plt.yscale('log')
plt.xscale('log')
