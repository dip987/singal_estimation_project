...
Params.num_s = 4
Params.r = r
sig_s = 0.01     # In meters
sig_w = to_rad(1.)  # In radians
...
for abs_w in w_space:
    ...
    w = abs_w_rad * np.array([1, 0, 0]).reshape(3, 1)
    ...
    for i in range(monte_carlo_trials):
        ...
        # Estimate
        w_estimate, phi, _ = estimate(generated_data, sig_w, sig_s, calculate_tensor_estimate=False)
...
plt.figure()
smooth_w_space = w_space

# Plot CRLB
plt.plot(smooth_w_space,
         [crlb_plot_wrapper(w, 'x', 0.01, sig_s, sig_w, Params.num_s, Params.num_w, 0, False) for w in
          smooth_w_space], linewidth=4)
plt.plot(smooth_w_space,
         [crlb_plot_wrapper(w, 'x', 0.01, sig_s, sig_w, Params.num_s, Params.num_w, 1, False) for w in
          smooth_w_space], linewidth=4)
plt.plot(smooth_w_space,
         [crlb_plot_wrapper(w, 'x', 0.01, sig_s, sig_w, Params.num_s, Params.num_w, 0, True) for w in
...
plt.plot(w_space, rmse_averaging[:, 0], '<')
plt.plot(w_space, rmse_averaging[:, 1], '>')
plt.plot(w_space, rmse_averaging[:, 2], '1')


plt.legend(['sqrt(CRLB) along x', 'sqrt(CRLB) along y', 'sqrt(saturated CRLB) along x', '$w_x MLE$', '$w_y MLE$', '$w_z MLE$', 'average w_x', 'average w_y',
            'average w_z'])
plt.yscale('log')
plt.xscale('log')
